...
public class mainTest {
	...
	public void main() {
		...
		for(int i=0;i<SIZE;i++){
			...
			assertTrue(Math.abs(network.getPageRank(i)-creator.getPageRank(i))<1e-4);
			//about 3% of mean probability. 
			//If it does not fail, they 																						
			//are obviously doing the same
			
			sum+=network.getPageRank(i);
		}
		assertTrue(Math.abs(sum-1)<1e-4);
		assertTrue(Math.abs(creator.getPageRank(0))<0.2);
